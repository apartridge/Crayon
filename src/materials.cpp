...
void makeGlassScene() 
{
	...
	Material* material0 = new Lambert(0.2*Vector3(1.0f, 1.0f, 1.0f));
    Material* material1 = new Lambert(0.9*Vector3(1.0f, 1.0f, 1.0f));
    Material* material2 = new Lambert(0.9*Vector3(0, 1.0f, 0));
    
	/*
	// Normal glass
	*/
	
	Material* normalGlass = new Glass();

	/*
	// Frosted glass
	*/

	Material* frostedGlass = new FrostedGlass(Vector3(0.5), Vector3(0.9));
	...
	*/
	...
	TriangleMesh * mesh = new TriangleMesh;
	mesh->setDefaultMaterial(material0);
	mesh->connectNameToMaterial("MaterialOne", frostedGlass);
	mesh->connectNameToMaterial("MaterialTwo", normalGlass);
	mesh->connectNameToMaterial("MaterialThree", materialWood3);
    mesh->load("monkey.obj");
	g_scene->addMesh(mesh);
